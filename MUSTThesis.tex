% LaTeX project: A template for MUST Thesis v0.1.0.7
%
% MUSTthesis (https://github.com/iihciyekub/mustthesis) bindings for XeLaTeX
% Author: Y.J.Li(iihciyekub)
% GitHub: https://github.com/iihciyekub/mustthesis
%
%
...
\def\projectBname{\texttt{A Template For MUST Thesis (v0.1.0.7 ,May,2019)}}
\def\projectSname{ \texttt{MUST Thesis (v0.1.0.7)}}


%% -------------------------------------------------<< 以下開始論文正文
\begin{document}

%% -------------------------------------------------<< \readme 为 MUST Thesis 模版说明,正式使用时注釋掉
...
% figure
\ff{folder2}
\node(\ff)[left=0pt of project.south west, yshift=-1cm,label={left:\color{blue}\texttt{\textbf{figure}}}]{{\Large \faFolderOpenO}};
\draw[line width=1pt,rounded corners=2pt,gray](project) |-(\ff);
...
\foreach \a /\b /\c  in {
	...
	175/\texttt{author.jpg}/\faFileImageO		
}
{
...
% reference
\ff{folder5}
\node(\ff)[left=0pt of project.south west, yshift=-6cm,label={left:\color{blue}\texttt{\textbf{reference}}}]{{\Large \faFolderOpenO}};
\draw[line width=1pt,rounded corners=1pt,gray](project) |-(\ff);

\foreach \a /\b /\c  in {
	25/\texttt{ch-ref.bib}/\faFileTextO,
	75/\texttt{en-ref.bib}/\faFileTextO
}
{
	\node(file\a)[left=0pt of \ff.south west,yshift=-\a * .1mm,label=left:\color{gray}\texttt{{\footnotesize \b}},black!70]{ \c};
	\draw[line width=1pt,rounded corners=2.25pt,gray](\ff) |-(file\a);
...
\node(df)[right=3cm of \ff.east]{\#~ \LaTeX 主文档};		
...
\node(df)[right=3cm of \ff.east]{\#~许可文件};
\draw[line width=1pt,rounded corners=1pt,gray](project) |-(\ff);
...
\node(\ff)[
		...
		black!70]
		{ \faFileTextO};
\node(df)[right=3cm of \ff.east]{\#~自动化编译工具};	
\draw[line width=1pt,rounded corners=1pt,gray](project) |-(\ff);
...
\bibreference{
	\bibliography{reference/ch-ref,reference/en-ref}
}
